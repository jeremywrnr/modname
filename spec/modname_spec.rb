...
describe Modname do
  def parse(str = '') = Modname::Driver.new.parse(str.split)
  def run(str = '') = Modname::Driver.new.run(str.split)
  def get(_str = '') = Modname::Driver.new
  ...
  context 'Runner' do
    ...
    it 'should call ext command with -e flag' do
      driver = get
      expect(driver).to receive(:exts).with(%w[txt md])
      driver.run ['-e', 'txt', 'md']
    end
    ...
    it 'should call regex command with file arguments' do
      driver = get
      expect(driver).to receive(:regex).with(%w[old new])
      driver.run %w[old new]
    end

    it 'should call regex command with flags' do
      driver = get
      expect(driver).to receive(:regex).with(%w[pattern replacement])
