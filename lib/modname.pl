...
# script to sequentially rename, remove or replace a phrase from files
# modname author: Jeremy Warner, sep 2014
my $debug = 0;
...
{
    (@ARGV > 1) || help();
    getopt('fmtesd', \%options);
    # target only some files
    # match some part of file name
    # replacement regex on match
    # change the file extensions
    # sequential name option
    # names based on last edit time
}

sub help
{
    print "modname - a tool to rename files (perl regex enabled)\n";
    print "usage: modname -f <FILETYPE>\n";
    print "\t-m:\t<MATCH-REGEX> [-t <TRANSFORM-REGEX>]\n";
    print "\t-e:\t<OLD> <NEW> (change extensions)\n";
    print "\t-s:\tname sequentially. ex:(-s a -> a1, a2, ...)\n";
    print "\t-d:\tassign edit datetimes to names\n";
    exit 1;
}
